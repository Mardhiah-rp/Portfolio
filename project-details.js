...
function buildCarousel(images, isMobile = false) {
  ...
  `).join("");
    ...
    // If mobile, wrap the entire carousel in a phone mock frame
    const carouselCore = `
...
function renderProject(project) {
    ...
    if (project.demoVideo) {
        const isPhoneFrame = (project.category === "mobile" || project.category === "design");
    ...
    document.getElementById("projectCarouselWrapper").innerHTML =
        buildCarousel(project.carouselImages, isMobile);

    // Tools (use your theme card style instead of bright list-group)
    document.getElementById("projectTools").innerHTML =
        project.tools.map(t => `<li class="list-group-item archive-list-item">${t}</li>`).join("");

    // Features
    document.getElementById("projectFeatures").innerHTML =
        project.features.map(f => `<li class="list-group-item archive-list-item">${f}</li>`).join("");

    // Links (only show if provided)
    const linksDiv = document.getElementById("projectLinks");
    const btns = [];

    // Use your theme classes (we’ll add these CSS classes right after)
    if (project.links?.live) {
        btns.push(`<a class="archive-link-btn" href="${project.links.live}" target="_blank" rel="noopener">Live Demo</a>`);
    }
    if (project.links?.github) {
        btns.push(`<a class="archive-link-btn" href="${project.links.github}" target="_blank" rel="noopener">GitHub</a>`);
    }
    if (project.links?.figma) {
        btns.push(`<a class="archive-link-btn" href="${project.links.figma}" target="_blank" rel="noopener">Figma</a>`);
    }
